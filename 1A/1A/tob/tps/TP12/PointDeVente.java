...
public class PointDeVente {
	...
	public int getPrix(Carburant carburant, LocalDateTime date) {
		if (prix.get(carburant)!=null) {
			if (prix.get(carburant).floorEntry(date)!=null) {
				if (prix.get(carburant).floorEntry(date).getValue()!=null) {
					return prix.get(carburant).floorEntry(date).getValue();
					}
				}
			}
		return 0;
