...
% fonction kppv.m
%--------------------------------------------------------------------------
function [Partition, confusion] = kppv_alex(DataA,DataT,labelA,K,ListeClass,labelT)

nb_classes = ListeClass(length(ListeClass));
confusion=zeros(nb_classes + 1); %%Sinon il y a un pb à la limite
[Na,~] = size(DataA);
[Nt,~] = size(DataT);
...
for i = 1:Nt_test
    ...
    % A COMPLÉTER
    Labels = zeros(1,nb_classes);  %%Dans labelA il y a le nombre d'individus
    for z=1:length(I)
