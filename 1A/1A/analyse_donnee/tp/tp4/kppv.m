...
% fonction kppv.m
%--------------------------------------------------------------------------
function [Partition,taux] = kppv(DataA,DataT,labelA,K,ListeClass,labelT)
confusion=zeros(K,K);
[Na,~] = size(DataA);
[Nt,~] = size(DataT);

Nt_test = Nt/20; % A changer, pouvant aller de 1 jusqu'à Nt
...
for i = 1:Nt_test
    ...
    for z=1:length(I)
        Labels(labelA(I(z))+1)=Labels(labelA(I(z))+1) + 1;
    end
    ...
    if etiquette~=labelT(i)
            confusion(labelT(i)+1,etiquette+1) = confusion(labelT(i)+1,etiquette+1) + 1;
    end
