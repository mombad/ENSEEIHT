...
procedure Specifier_Et_Tester is
    ...
    function Pgcd (A, B : in Integer) return Integer with
        Pre =>  A > 0 and B > 0,
        Post =>  Pgcd'Result >= 1
            and A mod Pgcd'Result = 0 -- Le Pgcd divise A            --Pgcd'Result correspond à ce que la fonction Pgcd retourne.
    ...
    -- Est-ce que le corps de cette procédure est correct ?  Pourquoi ?     Non car on ne respecte pas la précondition à cause du 0.
    -- Que donne son exécution ?          Le programme n'affiche rien et ne se termine pas.
    ...
    procedure Permuter (A, B: in out Integer) with
        Post => A = B'Old and B = A'Old     --A'Old est l'ancienne valeur de A et B'Old l'ancienne valeur de B
    is
        Copie_A : Integer;
    begin
        Copie_A := A;
